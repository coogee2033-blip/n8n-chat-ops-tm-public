...
console.log("OLV29 Auto-Reply AI Assistant v2.113 - inputGate via reply-target");
...
(() => {
  ...
  g.__chatopsResetStateOlv29 = () => {
    let removed = 0;
    const prefixes = ["autoFired::", "autoFired.", "chatops.queue.", "chatops.deferred.", "chatops.tabs.", "chatops.tabreg.", "_auto_last_sig", "olv29_auto_last_sig"];
  ...
  function bootQueueOnce() {
    ...
    try { migrateTabRegistryKey(); } catch (e) { console.warn("[OLV29] bootQueue migrateTabRegistryKey failed", e); }
    try { startTabRegistryHeartbeat(); } catch (e) { console.warn("[OLV29] bootQueue startTabRegistryHeartbeat failed", e); }
  ...
  function scanFramesWithInfo(label = "OLV29") {
    const frames = [];
    const docs = [];
  ...
  /** ===== 返信欄 ===== */
  function pickReplyTarget(label = "OLV29") {
  ...
  function mountAuto() {
    ...
    /** ===== DOM Ready Gates ===== */
  function waitForUiGate(initFn, label = "OLV29") {
    let started = false;
    let fired = false;
    const pollMs = 300;
  ...
  function waitForInputGate(startFn, label = "OLV29") {
    let fired = false;
  ...
  function exportDebug(label = "OLV29") {
  ...
  exportDebug("OLV29");
  waitForUiGate(() => initUI(), "OLV29");
  waitForInputGate(() => startAutomation(), "OLV29");
})();
