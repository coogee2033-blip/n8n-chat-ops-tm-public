...
(() => {
  ...
  /** ===== タブ間ロック機構（n8n 空ボディ対策） ===== */
  const N8N_LOCK_KEY = "olv29_n8n_send_lock";
  ...
  async function acquireN8nLock() {
    ...
    while (Date.now() - startWait < N8N_LOCK_WAIT_MS) {
      ...
      if (!lock || lock.until < now || lock.owner === n8nLockId) {
        if (setN8nLock(n8nLockId, now + N8N_LOCK_TTL_MS)) {
          console.log("[OLV29] n8n lock acquired:", n8nLockId);
          return true;
        }
      }
      // 他タブがロック中 → 待機
    ...
    console.warn("[OLV29] n8n lock timeout after", N8N_LOCK_WAIT_MS, "ms");
    return false;
  }
  ...
  function releaseN8nLock() {
    ...
    if (lock && lock.owner === n8nLockId) {
      clearN8nLock();
      console.log("[OLV29] n8n lock released:", n8nLockId);
    }
  }
    ...
    wrap.innerHTML = `
      ...
      <div style="padding:10px 12px; display:flex; flex-direction:column; gap:8px;">
        ...
        <div style="display:flex;align-items:center;gap:8px;">
          ...
          <input id="olv29_temp" type="range" min="0" max="2.0" step="0.1" value="1.0" style="flex:1;">
          <span id="olv29_temp_val" style="width:40px;text-align:right;font-size:12px;color:#ccc;">1.0</span>
        </div>
  ...
  function scrapeConversationStructured(rootOverride) {
    ...
    console.debug("[OLV29 v2.95] scrapeConversationStructured:", {
      total: all.length,
      male: maleCount,
      female: femaleCount,
    });
  ...
  async function buildWebhookPayload(isRegenerate = false) {
    ...
    // デバッグ用ログ
    console.debug("[OLV29 v2.95] buildWebhookPayload:", {
      blueStage,
    ...
    // デバッグ
    console.debug("[OLV29] payload extras: regenerate=", regenerate, "temp=", temperature, "oneLiner=", oneLinerPrompt?.slice(0, 30));
  ...
  async function sendManual() {
    ...
    try {
      console.log("[OLV29] sendManual: building payload (regenerate=true)...");
